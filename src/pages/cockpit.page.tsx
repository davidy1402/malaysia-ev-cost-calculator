    ...
    <div className="relative min-h-screen bg-background-default antialiased pb-[calc(100px+env(safe-area-inset-bottom))]">
      ...
      <main className="relative z-10 pt-[env(safe-area-inset-top)] px-base space-y-section-y">
        ...
        <header className="flex justify-between items-center py-base">
          <h1 className="text-h2 font-display text-text-primary tracking-tight">EV Calc MY</h1>
          <div className="flex space-x-3">
             <button onClick={() => setLanguage(language === 'en' ? 'zh' : 'en')} className="px-2 py-1 text-caption border border-border-subtle rounded text-text-secondary">
               {language.toUpperCase()}
             </button>
             <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="p-1 text-text-secondary">
        ...
        <section className="space-y-stack-md">
          <h2 className="text-body-lg text-text-primary font-semibold">Your EV & Journey</h2>
          
          <div className="bg-surface-base border border-border-subtle rounded-xl p-base space-y-stack-md">
            <div className="flex justify-between items-center">
              <span className="text-body text-text-secondary">Consumption (kWh/100km)</span>
            </div>
            ...
            <div className="flex overflow-x-auto space-x-tight pb-2 -mx-base px-base snap-x hide-scrollbar">
              {PRESETS.map(p => (
                <button
                  ...
                  className={`shrink-0 snap-start px-snug py-tight rounded-md border text-caption whitespace-nowrap transition-colors ${consumption === p.consumption ? 'bg-brand-primary text-text-inverse border-brand-primary' : 'bg-surface-overlay border-border-subtle text-text-secondary'}`}
                >
                  {p.name}
                </button>
              ))}
        ...
        <section className="space-y-stack-md">
           ...
           <div className="bg-surface-base border border-border-subtle rounded-xl p-base flex justify-between items-center">
             <div>
               <div className="text-caption text-text-secondary">Monthly Mileage</div>
               <div className="flex items-center space-x-2 mt-1">
                 ...
                 />
                 <span className="text-body text-text-secondary">km</span>
               </div>
             </div>
           ...
           <div className="bg-surface-base border border-border-subtle rounded-xl p-base flex justify-between items-center">
             <div>
               <div className="text-caption text-text-secondary">Home Electricity</div>
               <div className="flex items-center space-x-2 mt-1">
                 <input 
                   type="number" 
                   value={baselineKwh}
                   ...
                   className="bg-transparent text-h3 font-display tabular-nums text-text-primary w-20 outline-none"
                 />
                 <span className="text-body text-text-secondary">kWh</span>
               </div>
             </div>
             <button
                onClick={() => setHomeDisplayMode(homeDisplayMode === 'kwh' ? 'rm' : 'kwh')}
                className="px-3 py-1 text-caption bg-surface-overlay border border-border-subtle rounded text-text-secondary flex items-center"
              >
           ...
           <div className="bg-surface-base border border-border-subtle rounded-xl p-base flex justify-between items-center">
             <div>
               <div className="text-caption text-text-secondary">Monthly Petrol Spend</div>
               <div className="flex items-center space-x-2 mt-1">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-background-default/80 backdrop-blur-md border-t border-border-subtle p-base pb-[calc(16px+env(safe-area-inset-bottom))] flex justify-between items-center space-x-base">
        <button onClick={() => setIsDrawerOpen(true)} className="p-3 bg-surface-overlay border border-border-subtle rounded-lg text-text-primary active:scale-[0.98]">
          <Settings size={20} />
        </button>
        <button onClick={onCalculate} className="flex-1 bg-brand-primary text-text-inverse font-body font-semibold rounded-lg px-comfortable py-3 flex justify-center items-center space-x-2 active:scale-[0.98]">
          <span>Calculate</span>

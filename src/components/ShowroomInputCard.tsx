...
import { UserInputs, EvCalculationResult, SavedVehicle } from '../types/calculator';
import { Zap, Fuel, Home, Gauge, BookmarkPlus, Check, X } from 'lucide-react';
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="space-y-2 rounded-2xl border border-line bg-inset/50 p-3.5">
          ...
          <div className="flex flex-wrap gap-1 pt-0.5">
            ...
            <button
              ...
              title="根据每月油费推算的月行驶里程"
              className="rounded-full border border-oil/30 bg-oil-soft px-2 py-0.5 text-[10px] font-medium text-oil"
            >
              对齐油费 ({result.petrolEquivalentDistanceKm.toLocaleString()} km)
            </button>
          </div>
        </div>
        ...
        <div className="space-y-2 rounded-2xl border border-line bg-inset/50 p-3.5">
          ...
          <p className="text-[10px] leading-relaxed text-faint">
            按 TNB 费率反推 {result.baselineBill.kwh} kWh，账单 {formatRm(result.baselineBill.totalAmount)}
          </p>
        </div>
        ...
        <div className="space-y-2 rounded-2xl border border-line bg-inset/50 p-3.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] font-medium text-faint">现状基准</span>
          </div>
          ...
          <p className="flex items-center gap-1 text-[10px] text-faint">
            <Zap size={11} strokeWidth={1.75} className="text-oil" />
            约 {(inputs.fatherPetrolCostRm / inputs.petrolPricePerLiter).toFixed(1)} 公升 RON95
          </p>
        </div>
      </div>
    </div>
